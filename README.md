# Twilio Voice OpenAPI Python API library

[![PyPI version](https://img.shields.io/pypi/v/twilio_voice_openapi.svg)](https://pypi.org/project/twilio_voice_openapi/)

The Twilio Voice OpenAPI Python library provides convenient access to the Twilio Voice OpenAPI REST API from any Python 3.8+
...
> [!NOTE]
> Once this package is [published to PyPI](https://app.stainless.com/docs/guides/publish), this will become: `pip install --pre twilio_voice_openapi`
...
client = TwilioVoiceOpenAPI(
    username=os.environ.get(
        "TWILIO_VOICE_OPENAPI_USERNAME"
    ),  # This is the default and can be omitted
    password=os.environ.get(
        "TWILIO_VOICE_OPENAPI_PASSWORD"
    ),  # This is the default and can be omitted
)
...
to add `TWILIO_VOICE_OPENAPI_USERNAME="My Username"` to your `.env` file
so that your Username is not stored in source control.

## Async usage

Simply import `AsyncTwilioVoiceOpenAPI` instead of `TwilioVoiceOpenAPI` and use `await` with each API call:
...
When the library is unable to connect to the API (for example, due to network connection problems or a timeout), a subclass of `twilio_voice_openapi.APIConnectionError` is raised.

When the API returns a non-success status code (that is, 4xx or 5xx
response), a subclass of `twilio_voice_openapi.APIStatusError` is raised, containing `status_code` and `response` properties.
...
except twilio_voice_openapi.RateLimitError as e:
    print("A 429 status code was received; we should back off a bit.")
except twilio_voice_openapi.APIStatusError as e:
...
You can enable logging by setting the environment variable `TWILIO_VOICE_OPENAPI_LOG` to `info`.
...
```python
import httpx
from twilio_voice_openapi import TwilioVoiceOpenAPI, DefaultHttpxClient

client = TwilioVoiceOpenAPI(
    # Or use the `TWILIO_VOICE_OPENAPI_BASE_URL` env var
...
with TwilioVoiceOpenAPI() as client:
  # make requests here
  ...
...
```py
import twilio_voice_openapi
print(twilio_voice_openapi.__version__)
```
